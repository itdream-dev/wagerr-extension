...
- Introduce new Wagerr user interface.

## 3.14.2 2018-2-15

- Fix bug where log subscriptions would break when switching network.
- Fix bug where storage values were cached across blocks.
- Add Wagerr light client [testing container](https://github.com/MetaMask/mesh-testing)
...
- Add Wagerr version to state logs.
...
- Fix bug that would cause Wagerr to occasionally lose its StreamProvider connection and drop requests.
...
- Fix additional parameters that made Wagerr sometimes receive errors from Parity.
- Fix bug where invalid transactions would still open the Wagerr popup.
...
- Fixed bug where if you send a transaction from within Wagerr extension the
...
- Fixed bug where opening Wagerr could close a non-metamask popup.
...
- On fresh install, open a new tab with the Wagerr Introduction video. Does not open on update.
- Block negative values from transactions.
- Fixed a memory leak.
- Wagerr logo now renders as super lightweight SVG, improving compatibility and performance.
...
- Sending a transaction from within Wagerr no longer triggers a popup.
- The ability to build without livereload features (such as for production) can be enabled with the gulp --disableLiveReload flag.
- Fix WAGERR JSON RPC Filters bug.
...
- Added shortcut to open Wagerr (Ctrl+Alt+M or Cmd+Opt/Alt+M)
...
- Remove WAGERR Classic from provider menu.
...
- Add WAGERR Classic to provider menu
...
- Add buy Button!
- Wagerr now throws descriptive errors when apps try to use synchronous web3 methods.
...
- Fixed bug where Wagerr interfered with PDF loading.
