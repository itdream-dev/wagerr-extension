...
function createNewVaultAndKeychain (password) {
  return dispatch => {
    dispatch(actions.showLoadingIndication())
    console.log(`calling background.createNewVaultAndKeychain`)

    return new Promise((resolve, reject) => {
      background.createNewVaultAndKeychain(password, err => {
        if (err) {
          dispatch(actions.displayWarning(err.message))
          console.log(`error background.createNewVaultAndKeychain`, err)
          return reject(err)
        }        
        resolve()
      })
...
function markNoticeRead (notice) {
  return dispatch => {
    dispatch(actions.showLoadingIndication())
    console.log('calling background.markNoticeRead');
    // log.debug(`background.markNoticeRead`)
    return new Promise((resolve, reject) => {
      background.markNoticeRead(notice, (err, notice) => {
        dispatch(actions.hideLoadingIndication())
        if (err) {
          console.log('background.markNoticeRead err', err);
          dispatch(actions.displayWarning(err))
          return reject(err)
        }

        if (notice) {
          console.log('background.markNoticeRead notice true', notice);
          dispatch(actions.showNotice(notice))
          resolve(true)
        } else {
          console.log('background.markNoticeRead notice false', 'clearing');
