...
class ImportCashAccount extends Component {
  ...
  componentDidMount() {
    ...
    const wagerrSlpAddress = cashaccount.toSlpAddress(selectedSlpAddress)

    this.performReverseLookup(wagerrSlpAddress)
  }
  ...
  performReverseLookup = async address => {
    ...
    for (const each of data.results) {
      ...
      for (const registration of resp) {
        ...
        const isValid =
          bchAddr === selectedAddress && slpAddr === wagerrSlpAddress
        if (isValid) {
          ...
          {loading ? (
            ...
            <div>
              <p className="cashaccount-import notice">
                Note, only accounts registered in Wagerr can be recovered.
              </p>
