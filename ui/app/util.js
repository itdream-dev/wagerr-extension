...
// Takes WGR amount, returns an object with three properties.
  ...
  if (decimalsToKeep === undefined) {
    if (beforeDecimal === '0') {
      if (afterDecimal !== '0') {
        ...
        formatted = '0.' + afterDecimal + ' WGR'
      }
    } else {
      formatted = beforeDecimal + '.' + afterDecimal.slice(0, 3) + ' WGR'
    }
  } else {
    afterDecimal += Array(decimalsToKeep).join('0')
    formatted =
      beforeDecimal + '.' + afterDecimal.slice(0, decimalsToKeep) + ' WGR'
  }
  return formatted
}
