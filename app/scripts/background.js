 ...
 * @property {string} provider.type - An identifier for the type of network selected, allows Wagerr to use custom provider strategies for known networks.
 ...
 * @property {MetaMaskState} data - The data emitted from Wagerr controller, or used to initialize it.
 * @property {Number} version - The latest migration version that has been run.
 */

/**
 * Initializes the Wagerr controller, and sets up all platform configuration.
...
async function initialize () {
  ...
  // log.debug('Wagerr initialization complete.')
}
...
async function loadStateFromPersistence () {
  ...
  if (versionedData && !versionedData.data) {
    ...
    if (diskStoreState && diskStoreState.data) {
      ...
      raven.captureMessage(
        'Wagerr - Empty vault found - recovered from diskStore',
        {
      ...
      raven.captureMessage('Wagerr - Empty vault found - unable to recover')
    }
  }
  ...
  if (!versionedData) {
    throw new Error('Wagerr - migrator returned undefined')
  }
  ...
  if (localStore.isSupported) {
    ...
    setTimeout(() => {
      throw new Error('Wagerr - Localstore not supported')
    })
  }
...
/**
 * Initializes the Wagerr Controller with any initial state and default language.
...
function setupController (initState, initLangCode) {
  ...
  pump(
    ...
    error => {
      // log.error('Wagerr - Persistence pipeline failed', error)
    }
  )
  ...
  async function persistData (state) {
    if (!state) {
      throw new Error('Wagerr - updated state is missing', state)
    }
    if (!state.data) {
      throw new Error('Wagerr - updated state does not have data', state)
    }
    if (localStore.isSupported) {
      try {
  ...
  /**
   * Connects a Port to the Wagerr controller via a multiplexed duplex stream.
...
// On first install, open a window to Wagerr website to how-it-works.
// extension.runtime.onInstalled.addListener(function (details) {
//   if (details.reason === 'install' && !METAMASK_DEBUG) {
//     extension.tabs.create({ url: 'https://wagerrwallet.cash' })
//   }
// })
