...
module.exports = class NoticeController extends EventEmitter {
  ...
  markNoticeRead (noticeToMark, cb) {    
    console.log('markNoticeRead', 'called');
    cb = cb || function (err) { if (err) throw err }
    try {
      var notices = this.getNoticesList()
      console.log('markNoticeRead', notices);
      var index = notices.findIndex((currentNotice) => currentNotice.id === noticeToMark.id)
      console.log('markNoticeRead', index);
      notices[index].read = true
      notices[index].body = ''
      console.log('markNoticeRead', 'setNoticesList');
      this.setNoticesList(notices)
      console.log('markNoticeRead', 'calling getNextUnreadNotice');
