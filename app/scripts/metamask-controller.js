...
module.exports = class MetamaskController extends EventEmitter {
  ...
  async submitPassword (password) {
    ...
    await this.preferencesController.setAddresses(accounts, [])
   ...
   * Each strategy represents a different way of serializing an WAGERR key pair.
   *
  ...
  async signMessage (msgParams) {
    // log.info('WagerrController - signMessage')
  ...
  // ---------------------------------------------------------------------------
  // Wagerr Version 3 Migration Account Restauration Methods
  ...
  setupUntrustedCommunication (connectionStream, originDomain) {
    ...
    if (this.blacklistController.checkForPhishing(originDomain)) {
      // log.debug('Wagerr - sending phishing warning for', originDomain)
  ...
  setupProviderConnection (outStream, origin) {
    ...
    // // TODO: WGR and tokens middleware
  ...
  /**
   * A method for emitting the full Wagerr state to all registered listeners.
   * @private
   */
  privateSendUpdate () {
   ...
   * @param {string} rpcTarget - A URL for a valid WAGERR RPC API.
  ...
  /**
   * A method for recording whether the Wagerr user interface is open or not.
   * @private
   * @param {boolean} open
   */
